...
export const Calendario1 = ({ timeScale }: { timeScale: { interval: number; slotCount: number } }) =>{
      ...
      try {
        ...
        if (Array.isArray(data)) {
          const convertedData = data.map((appuntamento: any) => ({
            ...appuntamento,
            StartTime: new Date(appuntamento.AnnoS, appuntamento.MeseS , appuntamento.GiornoS, appuntamento.OraS, appuntamento.MinutoS),
            EndTime: new Date(appuntamento.AnnoE, appuntamento.MeseE , appuntamento.GiornoE, appuntamento.OraE, appuntamento.MinutoE),
          }));
  ...
  const editorFooter = (props: any):JSX.Element=>{
    return(
      <div className="e-popup-footer"><button className="e-event-details e-text-ellipsis e-control e-btn e-lib e-flat" title="Dettagli">Dettagli</button><button className="e-event-create e-text-ellipsis e-control e-btn e-lib e-flat e-primary" title="Salva">Salva</button></div>
    )
  }
  ...
  const onActionBegin = async (args: any) => {
    if (args.requestType === 'eventCreate') {
      ...
      const appuntamento = {
        Subject: newEvent.Subject || 'Nome mancante',
        Cognome: newEvent.Cognome || '',
        Cellulare: newEvent.Cellulare || '',
        Servizio: newEvent.Servizio || 'Servizio non specificato',
      ...
      const appuntamento = {
        ...
        Servizio: updatedEvent.Servizio || '',
  ...
  const onPopupOpen = (args: any) => {
    ...
    if (args.data.Cellulare !== undefined && args.type === 'QuickInfo') {
      ...
      if (timeDifference > 24 * 60 * 60 * 1000) {
        ...
        if (inputPhone === args.data.Cellulare || inputPhone==='p') {
  ...
  return (
    <ScheduleComponent currentView='WorkWeek' width='100%' height='100%' eventSettings={localData} editorTemplate={editorTable.bind(this)}  editorHeaderTemplate={editorHeader.bind(this)} timeScale={timeScale} quickInfoTemplates={{content: quickInfoContentTemplate}} popupOpen={onPopupOpen} allowDragAndDrop={true} actionBegin={onActionBegin} >
      <ViewsDirective>
        <ViewDirective option='WorkWeek' startHour='09:00' endHour='14:00' workDays={[6]}></ViewDirective>
        </ViewsDirective>
